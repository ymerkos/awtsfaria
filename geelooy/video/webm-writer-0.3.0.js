...
(function() {
    ...
    /**
	 * Convert the given canvas or RGBA image data to a WebP encoded image and return the image data as a string.
	 *
	 * @param {HTMLCanvasElement|ImageData|string} input - The canvas element, ImageData object, or a Data URL of type image/webp.
	 * @param {number} [quality=0.8] - The quality of the WebP image (from 0 to 1).
	 * @return {String} - The base64 encoded WebP image data.
	 */
	async function renderAsWebP(input, quality = 0.8) {
	    let frame;
	
	    if (typeof input === 'string' && /^data:image\/webp/.test(input)) {
	        // If input is a WebP Data URL, use it directly
	        frame = input;
	    } else if (input instanceof HTMLCanvasElement) {
	        // If input is a canvas, convert it to a WebP Data URL
	        frame = input.toDataURL('image/webp', quality);
	    } else if (input instanceof ImageData) {
	        // If input is an ImageData object, create a temporary canvas to handle it
	        const tempCanvas = new OffscreenCanvas(input.width, input.height)
			
	        const ctx = tempCanvas.getContext('2d');
	        ctx.putImageData(input, 0, 0);
	        frame = isOffscreen ? await (async () => {
					var blob = //await new Promise(r => {
						await tempCanvas.convertToBlob({type:"image/webp", quality})
					//})
					return new FileReaderSync().readAsDataURL(blob)
				})()
				: tempCanvas.toDataURL('image/webp', quality);
	    } else {
	        throw new Error('Input must be an HTMLCanvasElement, ImageData object, or a Data URL formatted string of type image/webp.');
	    }
    ...
    let WebMWriter = function(ArrayBufferDataStream, BlobBuffer) {
        return function(options) {
            ...
            this.addFrame = async function(frame, alpha, overrideFrameDuration) {
                ...
                let
                    keyframe = extractKeyframeFromWebP(await renderAsWebP(frame, options.quality)),
                ...
                addFrameToCluster({
                    ...
                    alpha: frameAlpha ? extractKeyframeFromWebP(await renderAsWebP(frameAlpha, options.alphaQuality)).frame : null
                });
            };
